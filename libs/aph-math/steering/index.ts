...
export const flee = (target: Vector, position: Vector, currentVelocity: Vector, maxVelocity: number): Vector => {
...
export const evade = (target: Vector, position: Vector, maxVelocity: number, currentVelocity: Vector, targetVelocity: Vector): Vector => {
	...
	return flee(targetFuturePosition, position, currentVelocity, maxVelocity);
};

export const pursuit = (target: Vector, position: Vector, maxVelocity: number, currentVelocity: Vector, targetVelocity: Vector): Vector => {
	let distance = target.subtract(position);
	let updatesNeeded = distance.magnitude() / maxVelocity;
	let tv = targetVelocity.multiply(updatesNeeded);
	let targetFuturePosition = target.add(tv);
	return seek(targetFuturePosition, position, currentVelocity, maxVelocity, 0);
};

export const wander = (velocity: Vector, wanderTarget: Vector, wanderRadius: number, wanderDistance: number,
...
export const follow = (position: Vector, currentVelocity: Vector, path: Path, context: PathContext, pointTolerance: number,
	...
	if (context.currentPointIndex === (path.segments.length - 1)) {
		...
		return seek(context.targetLocation, position, currentVelocity, maxVelocity, slowingRadius);
	} else {
		return seek(context.targetLocation, position, currentVelocity, maxVelocity, 0);
	}
};