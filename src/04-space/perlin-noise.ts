...
export class PerlinNoiseView extends ECSExample {
	...
	async load() {
		this.mapWidth = Math.floor(this.engine.app.canvas.clientWidth / this.mapCellSize);
		this.mapHeight = Math.floor(this.engine.app.canvas.clientHeight / this.mapCellSize);
	...
	onAssetsLoaded() {
		...
		this.engine.app.canvas.addEventListener('mousedown', this.onMouseClick);
	}
	...
	recreateMap() {
		...
		for (let i = 0; i < this.mapWidth; i++) {
			for (let j = 0; j < this.mapHeight; j++) {
				...
				sprite.rect(0, 0, this.mapCellSize, this.mapCellSize)
					.fill((elevation << 16) + (elevation << 8) + elevation);
	...
	destroy() {
		this.engine.app.canvas.removeEventListener('mousedown', this.onMouseClick);
	}
}
