...
import { AStarSearch, GridMap, MAP_TYPE_TILE, PathFinderContext } from '../../../../libs/pixi-math';
...
export class GameMap {
  ...
  initGridMap() {
    ...
    for (let [, block] of this.blocks) {
      if (block.type === MapBlocks.WALL) {
        this.gridMap.addObstruction(new ECSA.Vector(block.x, block.y));
      }
    }
  }
 ...
 /*
	 ...
	 */
  findNearestMapBlock(currentPos: ECSA.Vector, type: number): MapBlock {
    ...
    for (let block of allBlocks) {
      let dist = currentPos.manhattanDistance(new ECSA.Vector(block.x, block.y));
      if (dist < nearestDist) {
        nearest = block;
        nearestDist = dist;
      }
 ...
 /**
	 ...
	 */
  mapBlockToLocation(x: number, y: number): ECSA.Vector {
    return new ECSA.Vector((x * MAP_BLOCK_SIZE), (y * MAP_BLOCK_SIZE));
  }

 /**
	 * Transforms list of map-locations into list of world-locations
	 */
  mapBlockToLocations(input: Array<ECSA.Vector>): Array<ECSA.Vector> {
    let output = new Array<ECSA.Vector>();
 ...
 /**
	 ...
	 */
  locationToMapBlock(loc: ECSA.Vector): ECSA.Vector {
    let x = Math.floor(loc.x);
    let y = Math.floor(loc.y);

    return new ECSA.Vector(Math.floor(x / MAP_BLOCK_SIZE), Math.floor(y / MAP_BLOCK_SIZE));
  }
 ...
 /**
	 ...
	 */
  findPath(start: ECSA.Vector, goal: ECSA.Vector, outputPath: Array<ECSA.Vector>, directionPath: Array<ECSA.Vector>) {
    let ctx = new PathFinderContext();
    this.astar.search(this.gridMap, start, goal, ctx);
    let found = ctx.pathFound;

    let previous = new ECSA.Vector(-1);
    let current = new ECSA.Vector(-1);
    let index = 0;
