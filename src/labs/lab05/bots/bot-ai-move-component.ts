 ...
 */
export class BotMoveComponent extends FollowSteering {
  ...
  onUpdate(delta: number, absolute: number) {
    if (!this.pathFinished && this.path.segments.length > 2) {
      ...
      this.dynamics.aceleration = new ECSA.Vector(0, 0);
      this.dynamics.velocity = new ECSA.Vector(0, 0);
    }
  }

  goToPoint(startPos: ECSA.Vector, startLoc: ECSA.Vector, goal: ECSA.Vector) {

    let directionPath = new Array<ECSA.Vector>();
    let foundpath = new Array<ECSA.Vector>();
    ...
    for (let i = 0; i < locPath.length; i++) {
      let loc = locPath[i];
      locPath[i] = new ECSA.Vector(loc.x + MAP_BLOCK_SIZE / 2, loc.y + MAP_BLOCK_SIZE / 2);
    }
