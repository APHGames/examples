import { PerlinNoise } from '../../../libs/pixi-math/math/perlin-noise';
import { PathFinderContext, AStarSearch, GridMap, MAP_TYPE_OCTILE } from './../../../libs/pixi-math';
import * as ECSA from './../../../libs/pixi-component';

class ExamplePerlinNoise {
    ...
    constructor(view: HTMLCanvasElement, fullScreen: boolean = false) {
        ...
        this.mapWidth = Math.floor(view.clientWidth / this.mapCellSize);
        this.mapHeight = Math.floor(view.clientHeight / this.mapCellSize);
    ...
    onAssetsLoaded() {
        ...
        for (let i = 0; i < this.mapHeight; i++) {
            for (let j = 0; j < this.mapWidth; j++) {
              ...
              this.grid.setElevation(new ECSA.Vector(j, i), elevation); // max elevation is 255
    ...
    recreateMap() {
        ...
        for (let i = 0; i < this.mapWidth; i++) {
            for (let j = 0; j < this.mapHeight; j++) {
                let sprite = new ECSA.Graphics('');
                let pos = this.mapToWorld(i, j);
                sprite.position.set(pos.x, pos.y);
                let elevation = this.grid.getElevation(new ECSA.Vector(i, j));
    ...
    recalc(x: number, y: number) {
        ...
        let found = this.pathFinder.search(this.grid, new ECSA.Vector(0, 15), new ECSA.Vector(x, y), context);
...
new ExamplePerlinNoise(<HTMLCanvasElement>document.getElementById('gameCanvas'));