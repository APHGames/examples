...
export class ShaderNoise extends ECSExample {
	...
	async load() {
		...
		let vertexShader = await loadTextAsset('noise_vert', `${getBaseUrl()}/assets/07-graphics/shaders/noise.vert`);
		const fragmentShader = await loadTextAsset('noise_frag', `${getBaseUrl()}/assets/07-graphics/shaders/noise.frag`);
		...
		new ECS.Builder(this.engine.scene)
			...
			.withComponent(new ECS.FuncComponent('updater').doOnUpdate((cmp) => {
				(cmp.owner.asMesh().shader as any).resources.noiseUniforms.uniforms.u_time += 0.1;
			}))
