...
export default class SpriteAnimator extends BaseComponent {
  ...
  onUpdate(delta: number, absolute: number) {
    ...
    setTextureFrame(
      this.owner.pixiObj as { texture: import('pixi.js').Texture },
      getLoadedTexture(Assets.SPRITESHEET),
      this.spriteData.x + this.spriteData.w * (frameIndex % this.spriteData.frames_per_row),
      this.spriteData.y + this.spriteData.h * Math.floor(frameIndex / this.spriteData.frames_per_row),
      this.spriteData.w,
      this.spriteData.h,
    );
