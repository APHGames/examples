...
import { Assets, Attributes, Messages, UnitState, SpecFunctions, GameState } from '../constants';
...
export class PacmanController extends BaseComponent {
  ...
  onMessage(msg: ECSA.Message) {
    if(msg.action === Messages.PACMAN_KILLED) {
      ...
      setTextureFrame(this.owner.pixiObj as { texture: import('pixi.js').Texture }, getLoadedTexture(Assets.SPRITESHEET), spriteInfo.x, spriteInfo.y, spriteInfo.w, spriteInfo.h);
    }
  }
  ...
  protected followDirection(direction: Direction): boolean {
    ...
    this.owner.addComponentAndRun(new ECSA.ChainComponent()
      .call(() => this.currentWalkAnim = new PacmanWalkAnim(this.unit.pos, direction, this.model.isSomethingEatable(newPos), 150))
      .waitFor(() => this.currentWalkAnim)
      .call(() => {
        if(this.unit.state === UnitState.WALKING) {
          ...
          if(this.cmpState === ECSA.ComponentState.RUNNING) {
...
export class PacmanKeyController extends PacmanController {
  onUpdate(delta: number, absolute: number) {
    const cmpKey = this.scene.getGlobalAttribute<ECSA.KeyInputComponent>(Attributes.KEY_INPUT);
    if (!cmpKey) {
      return;
    }
