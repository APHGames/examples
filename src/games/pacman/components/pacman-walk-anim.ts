...
export default class PacmanWalkAnim extends BaseComponent {
  ...
  onUpdate(delta: number, absolute: number) {
    ...
    if(this.eatingAnim) {
      ...
      this.applyFrame(spriteData.x + frameIndex*spriteData.w, spriteData.y, spriteData.w, spriteData.h);
    } else {
      switch(this.direction) {
        case Direction.LEFT:
            this.applyFrame(8*32, 0, 32, 32);
          break;
        case Direction.RIGHT:
            this.applyFrame(0, 0, 32, 32);
          break;
        case Direction.DOWN:
            this.applyFrame(0, 32, 32, 32);
        break;
        case Direction.UP:
            this.applyFrame(8*32, 32, 32, 32);
          break;
      }
    }
  ...
  private applyFrame(x: number, y: number, w: number, h: number) {
    setTextureFrame(this.owner.pixiObj as { texture: import('pixi.js').Texture }, getLoadedTexture(Assets.SPRITESHEET), x, y, w, h);
  }
}