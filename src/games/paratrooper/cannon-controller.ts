...
export class CannonController extends ParatrooperBaseCmp {
  ...
  rotate(direction: number, delta: number) {
    ...
    // check boundaries
    pixiObj.rotation = Math.max(Math.min(pixiObj.rotation, DEG_TO_RAD * this.model.maxCannonAngle), DEG_TO_RAD * this.model.minCannonAngle);
  }
...
export class CannonInputController extends CannonController {
  onUpdate(delta: number, absolute: number) {
    const cmp = this.scene.getGlobalAttribute<ECSA.KeyInputComponent>(Attributes.KEY_INPUT);
    if (!cmp) {
      return;
    }
